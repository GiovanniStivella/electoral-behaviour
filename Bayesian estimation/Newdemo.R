...
one <- file.path("~/Electoral Behaviour/Bayesian estimation/Newdemo.stan")
...
summary_york <- as.data.frame(york$summary())
...
ed_simple <- lmer(y ~ per_vap_hisp+per_vap_white+bach+(1|GEOID20),
                  data = educ)
...
rest <- lmer(y ~ 0+per_vap_hisp+per_vap_white+bach+(1|GEOID20),
             data = restrict)
...
#COMPARISON
summary_york_c <- summary_york[3:102, 2]
scat <- cbind(ranef_rest, summary_york_c)

summary_york_f <- summary_york[115:117, 1:2]
fscat <- cbind(fixed_effects_rest, summary_york_f)

# Scatterplot
plot(scat$ranef_rest, scat$summary_york_c,
...
cor_value <- cor(scat$ranef_rest, scat$summary_york_c)
