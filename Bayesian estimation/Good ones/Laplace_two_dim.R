...
laplace_wide <- summary_laplace %>%
  select(1,2)%>%
  slice(4:nrow(summary_laplace))%>%
  separate(variable,
...
laplace_prec <- laplace_wide%>%slice(1:9167)

plot(laplace_prec$`value_1]`, laplace_prec$`value_2]`)

laplace_cand <- laplace_wide%>%slice(9168:9179)

plot(laplace_cand$`value_1]`, laplace_cand$`value_2]`)

colors <- ifelse(cand_long$candidate_number %% 2 == 1, "blue", "red")

text(
  laplace_cand$`value_1]`,
  laplace_cand$`value_2]`,
