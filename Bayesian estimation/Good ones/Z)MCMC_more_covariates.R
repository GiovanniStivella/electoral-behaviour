...
summary_cov <- as.data.frame(cov$summary())
...
cov_prec <- cov_wide%>%slice(1:100)

plot(cov_prec$`value_1]`, cov_prec$`value_2]`)

cov_cand <- cov_wide%>%slice(101:112)

colors <- ifelse(cand_long$candidate_number %% 2 == 1, "blue", "red")

plot(cov_cand$`value_1]`, cov_cand$`value_2]`)
